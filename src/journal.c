...
typedef struct wiggy_s
{
	...
	GFile *ofile;               // File to write to
	GFileOutputStream *ostream; // Output stream to write to the file
...
file_write_helper (GttGhtml *pl, const char *str, size_t len, gpointer data)
{
	...
	while (0 < buflen)
	{
		...
		if (0 > bytes_written)
		{
			g_warning("Failed to write to HTML file: %s", error->message);
			g_error_free (error);
			error = NULL;
			break;
		}
...
save_to_gio (Wiggy *wig, const char * filename)
{
	...
	else
	{
		...
		{
			g_warning("Failed to close the file \"%s\": %s",
								filename, error->message);
			g_error_free(error);
			error = NULL;
		}
                      ...
                      GtkHTMLStream *handle, gpointer data)
{
	...
	if (NULL == istream)
	{
		g_warning ("Failed to open HTML file \"%s\": %s", path, error->message);
		g_error_free (error);
		error = NULL;
...
#define BSZ 16000
	...
	while (TRUE)
	{
		const gssize bytes_read = g_input_stream_read (G_INPUT_STREAM(istream),
																									 buff, BSZ, NULL, &error);
		if (0 > bytes_read)
		{
			g_warning ("Failed to read HTML file \"%s\": %s", path, error->message);
			g_error_free (error);
			error = NULL;
			break;
		}
	...
	{
		g_warning ("Failed to close HTML file \"%s\": %s",
							 path, error->message);
		g_error_free (error);
		error = NULL;
	}
