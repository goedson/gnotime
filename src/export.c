...
struct export_format_s
{
	...
	GFile *ofile;                // Output file for exported data
	GFileOutputStream *ostream;  // Output stream to write exported data to
			  ...
			  export_format_t *xp)
{
	...
	while ((buflen > 0) && (0 <= bytes_written))
	{
		GError *error = NULL;
		bytes_written = g_output_stream_write(G_OUTPUT_STREAM(xp->ostream),
		                                      &str[off], buflen, NULL, &error);
		if (0 > bytes_written)
		{
			g_warning("Failed to write to export file: %s\n", error->message);
			g_clear_error(&error);
			break;
		}
		off += bytes_written;
...
export_really (GtkWidget *widget, export_format_t *xp)
{
	...
	{
		gchar *msg =
				g_strdup_printf(_("File %s could not be opened for export: %s"),
		                    xp->uri, error->message);
		g_clear_error(&error);
	...
	{
		g_warning("Failed to close export file: %s\n", error->message);
		g_clear_error(&error);
	}
